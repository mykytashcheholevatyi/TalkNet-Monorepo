...
update_backend_repo() {
    ...
    sudo mkdir -p $SCRIPT_DIR $DB_DIR $LOG_DIR
    sudo chown -R `whoami` $SCRIPT_DIR $DB_DIR $LOG_DIR
...
deploy_backend() {
    ...
    # Перезапуск Node.js сервера через pm2
...
# Перезапуск этого скрипта через pm2 для обеспечения его постоянного выполнения
pm2 start $SCRIPT_PATH --name update-backend-script

echo "Бэкенд успешно обновлен и перезапущен."
