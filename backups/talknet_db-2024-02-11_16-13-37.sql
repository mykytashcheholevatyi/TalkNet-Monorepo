...
COPY public.orders (id, buyer_id, product_id, quantity, total_price, status, created_at) FROM stdin;
\.
...
COPY public.products (id, seller_id, name, description, price, currency, created_at) FROM stdin;
\.
...
COPY public.users (id, username, email, password_hash, created_at, language, is_seller) FROM stdin;
\.
